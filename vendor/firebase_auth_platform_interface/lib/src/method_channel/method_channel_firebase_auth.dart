...
/// Returns `true` when the native auth [EventChannel] subscriptions
/// (`id-token` and `auth-state`) should be skipped. Only effective on Windows
/// when the host app has opted in via
/// [FirebaseAuthPlatform.disableIdTokenChannelOnWindows].
///
/// Both channels dispatch from a native background thread on Windows. The
/// current Flutter engine treats platform-channel traffic from a non-platform
/// thread as fatal (`fml::KillProcess` -> `__fastfail(FAST_FAIL_INVALID_ARG)`,
/// `0xC0000409`), which crashes the process right after Firebase Auth
/// initializes. Skipping both channels avoids the crash; `currentUser` stays in
/// sync via the method-channel responses of the auth operations.
///
/// Local fork patch. See firebase/flutterfire#18210 / #18226 /
/// flutter/flutter#134346.
bool _shouldSkipAuthEventChannels() {
...
class MethodChannelFirebaseAuth extends FirebaseAuthPlatform {
  ...
  MethodChannelFirebaseAuth({required FirebaseApp app})
    ...
    // On Windows the native `id-token`/`auth-state` EventChannels dispatch from
    // a background thread, which the current Flutter engine treats as fatal.
    // Skip both when opted in. See `_shouldSkipAuthEventChannels()`.
    if (!_shouldSkipAuthEventChannels()) {
