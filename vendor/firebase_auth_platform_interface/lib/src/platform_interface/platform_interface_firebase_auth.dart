...
abstract class FirebaseAuthPlatform extends PlatformInterface {
  ...
  /// When `true`, the native `id-token` and `auth-state` [EventChannel]
  /// subscriptions are skipped on Windows.
  ///
  /// Those channels dispatch from a native background thread, which the current
  /// Flutter engine treats as fatal (`__fastfail(FAST_FAIL_INVALID_ARG)`,
  /// `0xC0000409`), crashing the process right after Firebase Auth initializes.
  /// Opting in avoids the crash at the cost of `authStateChanges()` /
  /// `idTokenChanges()` not emitting on Windows; `currentUser` is still updated
  /// via the method-channel responses of auth operations.
  ///
  /// Local fork patch (not present in upstream 9.0.2). See
  /// firebase/flutterfire#18210 / #18226 / flutter/flutter#134346.
